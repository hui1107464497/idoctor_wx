...
class MHTabView: UIView ,UIScrollViewDelegate{
    static let kButtonWidth = UIScreen.main.bounds.size.width/3
...
//    public var titleArr:NSArray?
    ...
    public var addChildBlock : (()->(_:NSArray))? //添加自控制器
    
    
    
    public func createView(_ frame:CGRect, titleArr:NSArray) {
        if titleArr.count>0 {
            headScrollview = UIScrollView(frame:CGRect.init(x: 0, y: 0, width: kScreenWidth, height: headHeight))
            headScrollview.contentSize = CGSize.init(width: MHTabView.kButtonWidth * CGFloat(titleArr.count), height: headHeight)
            ...
            for i in 0 ..< titleArr.count  {
                let button = UIButton(frame: CGRect.init(x: CGFloat(i) * MHTabView.kButtonWidth, y: 0, width: MHTabView.kButtonWidth, height: headHeight-4))
...
//            }
    ...
    init(frame: CGRect , titleArr:NSArray) {
        ...
        self.createView(frame, titleArr: titleArr)
    }
